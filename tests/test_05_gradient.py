...
def test_full(ds_spec, ds_05_gradient_t, ds_05_gradient_dt_dtheta, ds_05_gradient_dt_dphi):
    transformation = construct_transformation(ds_05_gradient_dt_dphi)
    ds_grid = transformation.unfolded_spec_to_grid_full(ds_spec)
    ds_grid_selected = ds_grid.drop_vars((
        var for var in ds_grid
        if var.startswith('gt') or var.startswith('gp')
    ))
    common.test_function(
    ...
    ds_grid_selected = ds_grid.drop_vars((
        var for var in ds_grid
        if not var.startswith('gt')
    ))
    ds_grid_selected = ds_grid_selected.rename({
        var: var.replace('gt', '')
        for var in ds_grid_selected
    })
    common.test_function(
