...
class RichDaskProgressBar(dask.callbacks.Callback):
    ...
    def _start(self, _dsk):
        ...
        self.progress.start()
        self.task = self.progress.add_task(description='the task description', total=None)
    ...
    def update(self, state):
        ...
        num_tasks = sum(len(state[k]) for k in ["ready", "waiting", "running"]) + num_done
...
def open_ds_01_legendre(*, truncation, num_lat, num_lon, **_kwargs):
    return open_dataset(
        f'test_01_legendre/t{truncation}_{num_lat}_{num_lon}.nc'
    )
